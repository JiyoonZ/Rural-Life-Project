...
public class List extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		int blockSize = 10;		// 페이지바 제작 (출력되는 페이지 변수)
		...
		if ( n == 1 ) {
			...
			pagebar += String.format(" <li class='page-item'><a class='page-link' href='/rural/notice/list.do?page=%d&category=%s&sort=%s&column=%s&search=%s' tabindex='-1'><img src='/rural/assets/img/logo/previous.png' style='width:14px;'/></a></li> ", n-1, category, sort, column, search);
		}
		...
		while ( !(loop > blockSize || n > totalPage ) ) {
			if ( n == nowPage ) {
				...
				pagebar += String.format(" <li class='page-item'><a class='page-link' href='/rural/notice/list.do?page=%d&category=%s&sort=%s&column=%s&search=%s' tabindex='-1'>%d</a></li> ", n, category, sort, column, search, n);
			}
		...
		if ( n > totalPage ) {
			...
			pagebar += String.format(" <li class='page-item'><a class='page-link' href='/rural/notice/list.do?page=%d&category=%s&sort=%s&column=%s&search=%s' tabindex='-1'><img src='/rural/assets/img/logo/next.png' style='width:14px;'/></a></li> ", n, category, sort, column, search);
		}
